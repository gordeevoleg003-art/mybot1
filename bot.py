...
# ========== КОНФИГУРАЦИЯ ==========
TOKEN = "8867087634:AAGehtjfnpQE4SRNh0SyXn51Hi8SuVSBUmY"  # ← ОБЯЗАТЕЛЬНО СМЕНИТЕ!
ADMIN_ID = 6548175188
CHAT_ID = -1001234567890  # ← впишите реальный ID, если нужна казна
    ...
    if user:
        if username:
            try:
                ...
                print(f"⚠️ UPDATE username: {e}")
        return user
    ok = create_user(user_id, username)
    if not ok:
        print(f"❌ create_user({user_id}) вернул False")
    for _ in range(5):
        user = get_user(user_id)
        if user:
            return user
        time.sleep(0.1)
    print(f"❌ Не смог создать/найти {user_id} после 5 попыток")
    return None
...
def get_user_id_by_username(username):
    try:
        ...
        print(f"⚠️ get_user_id_by_username: {e}")
        return None
                    ...
                    try:
                            ...
                            if "text" in msg:
                                ...
                                if cmd is not None:
                                    if cmd == "/start":
                                        ...
                                        send_message(chat_id, f"<code>{DB_PATH}</code>")
                                    elif cmd == "/whoami":
                                        send_message(chat_id,
                                            f"user_id: <code>{user_id}</code>\n"
                                            f"chat_id: <code>{chat_id}</code>\n"
                                            f"ADMIN_ID: <code>{ADMIN_ID}</code>\n"
                                            f"Ты админ: {'ДА' if user_id == ADMIN_ID else 'НЕТ'}")
                                    elif cmd == "/testdb":
                                        if user_id != ADMIN_ID:
                                            send_message(chat_id, "Нет прав")
                                        else:
                                            import traceback as _tb
                                            res = [f"DB_PATH: <code>{DB_PATH}</code>"]
                                            res.append(f"Файл существует: {os.path.exists(DB_PATH)}")
                                            if os.path.exists(DB_PATH):
                                                res.append(f"Размер: {os.path.getsize(DB_PATH)} байт")
                                            try:
                                                with _conn() as conn:
                                                    c = conn.cursor()
                                                    c.execute("SELECT COUNT(*) as n FROM users")
                                                    res.append(f"Юзеров в БД: {c.fetchone()['n']}")
                                            except Exception as e:
                                                res.append(f"❌ SELECT: {e}")
                                            test_id = 999999999999
                                            try:
                                                with DB_LOCK:
                                                    with _conn() as conn:
                                                        conn.execute("INSERT OR IGNORE INTO users (user_id, username, balance, last_bonus, last_active) VALUES (?, ?, ?, ?, ?)",
                                                                     (test_id, "test_user", 100, db_now(), db_now()))
                                                        conn.commit()
                                                res.append("✅ INSERT: ок")
                                                u = get_user(test_id)
                                                res.append(f"✅ Проверка: {'ок' if u else 'НЕТ'}")
                                                with DB_LOCK:
                                                    with _conn() as conn:
                                                        conn.execute("DELETE FROM users WHERE user_id = ?", (test_id,))
                                                        conn.commit()
                                                res.append("✅ DELETE: ок")
                                            except Exception as e:
                                                res.append(f"❌ INSERT/тест: {e}")
                                                res.append("<code>" + _tb.format_exc()[-400:] + "</code>")
                                            send_message(chat_id, "\n".join(res))
                                    elif cmd == "/debug_give":
                                        if user_id != ADMIN_ID:
                                            send_message(chat_id, "Нет прав")
                                        elif len(args) != 2:
                                            send_message(chat_id, "Формат: /debug_give ID сумма")
                                        else:
                                            out = [f"args[0]=<code>{args[0]}</code>", f"args[1]=<code>{args[1]}</code>"]
                                            try:
                                                tid = int(args[0])
                                                out.append(f"target_id={tid}")
                                                u = get_user(tid)
                                                out.append(f"В БД: {'ЕСТЬ, username=' + str(u['username']) if u else 'НЕТ'}")
                                                amt = int(args[1])
                                                out.append(f"amount={amt}")
                                                if not u:
                                                    ok = create_user(tid)
                                                    out.append(f"create_user={ok}")
                                                    u2 = get_user(tid)
                                                    out.append(f"после create: {'ЕСТЬ' if u2 else 'НЕТ'}")
                                            except Exception as e:
                                        ...
                                        if user_id == ADMIN_ID and len(args) == 1:
                                            try:
                                                ...
                                                name = u["username"] if u and u["username"] else str(target)
                                                if not titles:
                                                    send_message(chat_id, str(name) + " — титулов нет")
                                                else:
                                                    txt = str(name) + "\n\n"
                                                    for t in titles:
                                                        txt += "  " + t["name"] + "\n"
                                            ...
                                            send_message(chat_id, "/user_titles ID")
                                    elif cmd == "/give_title":
                                        if user_id != ADMIN_ID:
                                            send_message(chat_id, "Нет прав!")
                                        elif len(args) != 2:
                                            send_message(chat_id, "/give_title ID КЛЮЧ")
                                        else:
                                            try:
                                                target = int(args[0])
                                                title_key = args[1].lower().strip()
                                                success, result = award_title(target, title_key, awarded_by=user_id)
                                                send_message(chat_id, result)
                                            except ValueError:
                                                send_message(chat_id, "ID!")
                                    elif cmd == "/give_custom_title":
                                        if user_id != ADMIN_ID:
                                            send_message(chat_id, "Нет прав!")
                                        elif len(args) < 2:
                                            send_message(chat_id, "/give_custom_title ID ТЕКСТ")
                                        else:
                                            try:
                                                target = int(args[0])
                                                ...
                                                if not custom_text:
                                                    ...
                                                    send_message(chat_id, "Макс 50!")
                                                else:
                                                    success, result = award_title(target, "custom", custom_text=custom_text, awarded_by=user_id)
                                                    send_message(chat_id, result)
                                            except ValueError:
                                                send_message(chat_id, "ID!")
                                    elif cmd == "/remove_title":
                                        if user_id != ADMIN_ID:
                                            send_message(chat_id, "Нет прав!")
                                        elif len(args) != 2:
                                            send_message(chat_id, "/remove_title ID КЛЮЧ")
                                        else:
                                            try:
                                                target = int(args[0])
                                                title_key = args[1].lower().strip()
                                                if remove_title(target, title_key):
                                                    send_message(chat_id, "Снят с " + str(target))
                                                else:
                                                    send_message(chat_id, "Не найден!")
                                            except ValueError:
                                                send_message(chat_id, "ID!")
                                        ...
                                        if user_id == ADMIN_ID and len(args) == 2:
                                            try:
                                                ...
                                                if rate > 0 and az_amt > 0:
                                                    set_az_rate(rate)
                                                    set_az_amount(az_amt)
                                                    send_message(chat_id, format(rate, ",") + " → " + str(az_amt) + " Az")
                                            except Exception:
                                                send_message(chat_id, "/set_az_rate ПОИНТЫ AZ")
                                    elif cmd == "/cases":
                                        ...
                                        if user_id == ADMIN_ID and len(args) == 1:
                                            try:
                                                ...
                                                if 0 <= n <= 36:
                                                    rl_set_rig(n)
                                                    send_message(chat_id, "Следующее: " + str(n))
                                                else:
                                                    send_message(chat_id, "0-36!")
                                            except Exception:
                                                send_message(chat_id, "/rl_rig NUMBER")
                                    elif cmd == "/rl_reset":
                                        if user_id == ADMIN_ID:
                                            rl_set_rig(None)
                                            send_message(chat_id, "Сброшено")
                                    elif cmd == "/rl_off":
                                        if user_id == ADMIN_ID:
                                            set_roulette_enabled(False)
                                            send_message(chat_id, "Рулетка ВЫКЛ")
                                    elif cmd == "/rl_on":
                                        if user_id == ADMIN_ID:
                                            set_roulette_enabled(True)
                                            send_message(chat_id, "Рулетка ВКЛ")
                                    elif cmd == "/rl_status":
                                        if user_id == ADMIN_ID:
                                            rig = rl_get_rig()
                                            send_message(chat_id, "Рулетка: " + ("ON" if is_roulette_enabled() else "OFF") + "\nПодкрутка: " + (str(rig) if rig is not None else "нет") + "\nБлоков: " + str(len(rl_blocklist())))
                                    elif cmd == "/rl_block":
                                        if user_id == ADMIN_ID and len(args) == 1:
                                            try:
                                                target = int(args[0])
                                                rl_block_user(target)
                                                send_message(chat_id, "Заблокирован: " + str(target))
                                            except Exception:
                                                send_message(chat_id, "/rl_block ID")
                                    elif cmd == "/rl_unblock":
                                        if user_id == ADMIN_ID and len(args) == 1:
                                            try:
                                                target = int(args[0])
                                                if rl_unblock_user(target):
                                                    send_message(chat_id, "Разблокирован: " + str(target))
                                                else:
                                                    send_message(chat_id, "Не был заблокирован!")
                                            except Exception:
                                                send_message(chat_id, "/rl_unblock ID")
                                    elif cmd == "/promo":
                                        if len(args) >= 1:
                                    ...
                                    elif cmd == "/promo_create":
                                        if user_id == ADMIN_ID:
                                            full = text.split(None, 1)
                                            if len(full) > 1:
                                                data_parts = full[1].split('|')
                                                if len(data_parts) >= 2:
                                                    try:
                                                        code = data_parts[0].strip()
                                                        reward = int(data_parts[1].strip())
                                                        max_uses = int(data_parts[2].strip()) if len(data_parts) > 2 else 1
                                                        success, result = create_promocode(code, reward, max_uses)
                                                        send_message(chat_id, result)
                                                    except Exception:
                                                        send_message(chat_id, "Формат!")
                                        ...
                                        if user_id == ADMIN_ID:
                                            handle_promo_list(chat_id, None, user_id)
                                    elif cmd == "/promo_toggle":
                                        if user_id == ADMIN_ID and len(args) == 1:
                                            success, result = toggle_promocode(args[0])
                                            send_message(chat_id, result)
                                    elif cmd == "/promo_delete":
                                        if user_id == ADMIN_ID and len(args) == 1:
                                            if delete_promocode(args[0]):
                                                send_message(chat_id, "Удалён!")
                                            else:
                                                send_message(chat_id, "Не найден!")
                                    elif cmd == "/give":
                                        if user_id != ADMIN_ID:
                                            ...
                                            send_message(chat_id, "/give ID Сумма")
                                        else:
                                            target_id = None
                                            if args[0].startswith('@'):
                                                target_id = get_user_id_by_username(args[0][1:])
                                                if not target_id:
                                                    send_message(chat_id, "@username не найден!")
                                            else:
                                                try:
                                            ...
                                            if target_id:
                                                try:
                                                    ...
                                                    send_message(chat_id, "Сумма!")
                                    elif cmd == "/users":
                                        if user_id == ADMIN_ID:
                                            handle_admin_users_top(chat_id, None, user_id)
                                    elif cmd == "/treasury_add":
                                        if user_id == ADMIN_ID and len(args) >= 2:
                                            try:
                                                amount = int(args[0])
                                                reason = " ".join(args[1:])
                                                with DB_LOCK:
                                                    with _conn() as conn:
                                                        ...
                                                        c.execute("INSERT INTO transactions (user_id, amount, type, game, details) VALUES (?, ?, ?, ?, ?)",
                                                                  (ADMIN_ID, amount, "treasury_admin_add", "treasury", reason))
                                                        conn.commit()
                                                send_message(chat_id, "+" + format(amount, ",") + ". Казны: " + format(get_treasury(CHAT_ID), ","))
                                            except Exception:
                                                send_message(chat_id, "Формат!")
                                    elif cmd == "/treasury_give":
                                        if user_id == ADMIN_ID and len(args) >= 2:
                                            try:
                                                target_id = int(args[0])
                                                amount = int(args[1])
                                        ...
                                        if user_id == ADMIN_ID and len(args) == 1:
                                            try:
                                                success, result = set_tax_percent(int(args[0]))
                                                send_message(chat_id, result)
                                            except Exception:
                                    ...
                                    if action == "crash_amount":
                                        ...
                                        process_promo_create(chat_id, user_id, text)
                                    elif action == "admin_promo_toggle":
                                        process_admin_promo_toggle(chat_id, user_id, text)
                                    elif action == "admin_promo_delete":
                                        process_admin_promo_delete(chat_id, user_id, text)
                                    elif action == "admin_rl_rig":
                                        process_admin_rl_rig(chat_id, user_id, text)
                                    elif action == "admin_rl_block":
                                        process_admin_rl_block(chat_id, user_id, text)
                                    elif action == "admin_rl_unblock":
                                        process_admin_rl_unblock(chat_id, user_id, text)
                                    elif action == "admin_set_az_rate":
                                        process_admin_set_az_rate(chat_id, user_id, text)
                                    elif action == "admin_set_tax":
                                        process_admin_set_tax(chat_id, user_id, text)
                                    elif action == "admin_give_money":
                                        process_admin_give_money(chat_id, user_id, text)
                                    elif action == "admin_user_info":
                                        process_admin_user_info(chat_id, user_id, text)
                                    elif action == "admin_treasury_add":
                                        process_admin_treasury_add(chat_id, user_id, text)
                                    elif action == "admin_treasury_give":
                                        process_admin_treasury_give(chat_id, user_id, text)
                                    elif action == "admin_title_give":
                                        process_admin_title_give(chat_id, user_id, text)
                                    elif action == "admin_title_give_custom":
                                        process_admin_title_give_custom(chat_id, user_id, text)
                                    elif action == "admin_title_remove":
                                        process_admin_title_remove(chat_id, user_id, text)
                                    elif action == "admin_user_titles":
                                        process_admin_user_titles(chat_id, user_id, text)
                        ...
                        if "callback_query" in update:
                            ...
                            if data == "back":
                                ...
                                handle_admin_titles(chat_id, message_id, user_id)
                            elif data.startswith("admin_title_info_"):
                                handle_admin_title_info(chat_id, message_id, user_id, data.replace("admin_title_info_", ""))
                            elif data == "admin_title_give":
                                handle_admin_title_give(chat_id, message_id, user_id)
                            elif data == "admin_title_give_custom":
                                handle_admin_title_give_custom(chat_id, message_id, user_id)
                            elif data == "admin_title_remove":
                                handle_admin_title_remove(chat_id, message_id, user_id)
                            elif data == "admin_user_titles":
                                handle_admin_user_titles(chat_id, message_id, user_id)
                            elif data == "dice":
                                ...
                                handle_admin_economy(chat_id, message_id, user_id)
                            elif data == "admin_roulette_menu":
                                handle_admin_roulette_menu(chat_id, message_id, user_id)
                            elif data == "admin_promo_menu":
                                handle_admin_promo_menu(chat_id, message_id, user_id)
                            elif data == "admin_players":
                                handle_admin_players(chat_id, message_id, user_id)
                            elif data == "admin_treasury_menu":
                                handle_admin_treasury_menu(chat_id, message_id, user_id)
                            elif data == "admin_toggle_az":
                                handle_admin_toggle_az(chat_id, message_id, user_id)
                            elif data == "admin_set_az_rate":
                                handle_admin_set_az_rate(chat_id, message_id, user_id)
                            elif data == "admin_toggle_roulette":
                                handle_admin_toggle_roulette(chat_id, message_id, user_id)
                            elif data == "admin_rl_rig":
                                handle_admin_rl_rig(chat_id, message_id, user_id)
                            elif data == "admin_rl_reset":
                                handle_admin_rl_reset(chat_id, message_id, user_id)
                            elif data == "admin_rl_block":
                                handle_admin_rl_block(chat_id, message_id, user_id)
                            elif data == "admin_rl_unblock":
                                handle_admin_rl_unblock(chat_id, message_id, user_id)
                            elif data == "admin_rl_blocklist":
                                handle_admin_rl_blocklist(chat_id, message_id, user_id)
                            elif data == "admin_rl_status":
                                handle_admin_rl_status(chat_id, message_id, user_id)
                            elif data == "admin_promo_create":
                                handle_promo_create(chat_id, message_id, user_id)
                            elif data == "admin_promo_list":
                                handle_promo_list(chat_id, message_id, user_id)
                            elif data == "admin_promo_toggle":
                                handle_admin_promo_toggle(chat_id, message_id, user_id)
                            elif data == "admin_promo_delete":
                                handle_admin_promo_delete(chat_id, message_id, user_id)
                            elif data == "admin_give_money":
                                handle_admin_give_money(chat_id, message_id, user_id)
                            elif data == "admin_users_top":
                                handle_admin_users_top(chat_id, message_id, user_id)
                            elif data == "admin_set_tax":
                                handle_admin_set_tax(chat_id, message_id, user_id)
                            elif data == "admin_user_info":
                                handle_admin_user_info(chat_id, message_id, user_id)
                            elif data == "admin_treasury_add":
                                handle_admin_treasury_add(chat_id, message_id, user_id)
                            elif data == "admin_treasury_give":
                                handle_admin_treasury_give(chat_id, message_id, user_id)
                            elif data == "admin_treasury_logs":
                                handle_admin_treasury_logs(chat_id, message_id, user_id)
                            elif data == "admin_treasury_stats":
                                handle_admin_treasury_stats(chat_id, message_id, user_id)
                            elif data == "az_menu":
                                handle_az_menu(chat_id, message_id, user_id)
                            elif data == "az_ex_1":
...
if __name__ == "__main__":
    ...
    if os.environ.get("KEEP_ALIVE") == "1":
        time.sleep(3600)